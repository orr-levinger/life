...
class VisionSensor(Sensor):
    ...
    def get_reading(self, creature: Creature, world: World) -> Dict[str, str]:
        ...
        if y + 1 < world.height:
            ...
            # Check for food one cell north (using int coordinates)
            elif any(int(f.x) == x and int(f.y) == y + 1 for f in world.foods):
                readings["north"] = "food"
        # South
        if y - 1 >= 0:
            if any(int(c.x) == x and int(c.y) == y - 1 for c in world.creatures):
                readings["south"] = "creature"
            # Check for food one cell south (using int coordinates)
            elif any(int(f.x) == x and int(f.y) == y - 1 for f in world.foods):
                readings["south"] = "food"
        # East
        if x + 1 < world.width:
            ...
            # Check for food one cell east (using int coordinates)
            elif any(int(f.x) == x + 1 and int(f.y) == y for f in world.foods):
                readings["east"] = "food"
        # West
        if x - 1 >= 0:
            if any(int(c.x) == x - 1 and int(c.y) == y for c in world.creatures):
                readings["west"] = "creature"
            # Check for food one cell west (using int coordinates)
            elif any(int(f.x) == x - 1 and int(f.y) == y for f in world.foods):
                readings["west"] = "food"
        return readings
