...
class TestParentChildRelationship(unittest.TestCase):
    def test_parent_does_not_attack_child(self):
        ...
        non_child = Creature(
            parent.x + 0.1, parent.y, size=1.0, energy=10.0, brain=None
        )
        ...
        print(
            f"Distance: {math.sqrt((parent.x - non_child.x)**2 + (parent.y - non_child.y)**2)}"
        )
        print(
            f"Attack range: {(parent.radius + non_child.radius) * parent.ATTACK_RANGE_FACTOR}"
        )

        # Ensure deterministic rest/wander decision
        random.seed(0)
    ...
    def test_child_does_not_attack_parent(self):
        ...
        print(
            f"Distance: {math.sqrt((child.x - non_parent.x)**2 + (child.y - non_parent.y)**2)}"
        )
        print(
            f"Attack range: {(child.radius + non_parent.radius) * child.ATTACK_RANGE_FACTOR}"
        )
